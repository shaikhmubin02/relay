...
*Draft for builder.aws. Publish under your own account with the title above. "Agents for Humans" has to be in the title. Re-check every number against `eval/results/latest.json` before publishing.*
...
Untrue. Three people held it. All three had been asked and none had replied. The planner branched on *"does this shift require a certification"* rather than on *what actually blocked this gap*, and produced a confident, well-formed, wrong sentence. That is exactly the failure mode that makes a coordinator stop trusting the whole system.
